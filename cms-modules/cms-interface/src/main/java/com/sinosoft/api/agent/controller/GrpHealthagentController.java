...
public class GrpHealthagentController extends BaseController {
    ...
    public Map<String, Object> assignOrphanPolicy(@RequestBody Map<String, Object> requestMap) {
        ...
        // 获取团单号列表
        List<Map<String, String>> grpContNos = (List<Map<String, String>>) inputData.get("lAAgents");
        // 模拟业务处理逻辑
        log.info("处理同步信息, 数量: {}", grpContNos.size());
        for (Map<String, String> item : grpContNos) {
            log.info("业务员代码: {},业务员姓名: {}", item.get("agentcode"), item.get("name"));
            // 在此处添加实际的业务逻辑
        }
        // 构建响应
