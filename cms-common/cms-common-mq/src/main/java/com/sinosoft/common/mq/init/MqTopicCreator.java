...
public class MqTopicCreator implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        ...
        for (MqListenerAnnotationBeanPostProcessor.TopicDefinition def : topics) {
            try {
                if (def.getClusterName() != null) {
                    // 使用指定的集群
                    mqFactory.createTopic(def.getTopic(), def.getMqType(), def.getClusterName());
                } else {
                    // 使用默认集群
                    switch (def.getMqType()) {
                        case KAFKA:
                            mqFactory.createTopic(def.getTopic(), MqCluster.defaultKafka());
                            break;
                        case RABBIT_MQ:
                            mqFactory.createTopic(def.getTopic(), MqCluster.defaultRabbit());
                            break;
                        case ROCKET_MQ:
                            mqFactory.createTopic(def.getTopic(), MqCluster.defaultRocket());
                            break;
                    }
                }
            } catch (Exception e) {
