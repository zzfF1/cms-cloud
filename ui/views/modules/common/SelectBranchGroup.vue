<template>
  ...
  <el-row>
    <el-dialog v-model="visible" title="选择销售机构" width="60%" top="2vh" append-to-body>
      <el-form ref="queryFormRef" :model="queryParams" label-width="90px" size="small">
        <el-row>
          <el-col :span="6">
      ...
      <el-table ref="tableRef" v-loading="loading" :data="branchGroupList" border height="400px" show-overflow-tooltip @select="clickRow">
        ...
        <el-table-column label="管理机构" prop="manageComName" width="300">
          <template #default="scope">
            <div>{{ scope.row.managecom }}-{{ scope.row.managecomname }}</div>
          </template>
        </el-table-column>
        <el-table-column label="销售机构" prop="branchattr" width="350">
          <template #default="scope">
